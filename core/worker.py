...
class Worker:
    ...
    def start(self):
        ...
        wordlist_info = wordlist.Wordlist(self.wordlist, self.cluster_count)
        wordlist_lists = wordlist_info.load()
        ...
        for depth in range(0, self.subdomain_depth):
            wordlists = wordlists + ['.'.join(p) for p in permutations([ x for x in wordlist_lists if x is not 'www' ], r=depth+1)]
